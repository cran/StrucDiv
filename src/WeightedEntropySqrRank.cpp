...
// [[Rcpp::export(name=".WeightedEntropySqrRank")]]
NumericVector WeightedEntropySqrRank( NumericMatrix Hetx, List PMat,
                                      bool narm,
      ...
      if(xVal_.length()==0) {
        ...
        for (int m = 0; m < xPMatSub.nrow(); m++) {
          for( int n = 0; n < xPMatSub.ncol(); n++) {
            ...
            else EntMat(m,n) = pow( ValPos[m] - ValPos[n], 2 )*( xPMatSub(m,n) * (-log(xPMatSub(m,n))) );
