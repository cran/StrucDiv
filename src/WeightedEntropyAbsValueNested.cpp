...
// [[Rcpp::export(name=".WeightedEntropyAbsValueNested")]]
NumericVector WeightedEntropyAbsValueNested( NumericMatrix Hetx, 
      ...
      if(xVal_.length()==0) {
        ...
        for (int m = 0; m < xPMatSub.nrow(); m++) {
          for( int n = 0; n < xPMatSub.ncol(); n++) {
            ...
            else EntMat(m,n) = std::fabs( xVal[m] - xVal[n] )*( xPMatSub(m,n) * (-log(xPMatSub(m,n))) );
